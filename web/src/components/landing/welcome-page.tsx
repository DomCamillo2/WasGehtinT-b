...
import { DiscoverMotionRoot, m, useReducedMotion, dur, easeOut } from "@/lib/discover-motion";
import { SITE_LOGO_SRC } from "@/lib/site-config";

const WelcomeScrollMount = dynamic(
  () => import("./welcome-scroll-mount").then((mod) => mod.WelcomeScrollMount),
  { ssr: false },
);

/** Full-bleed Tübingen nightlife atmosphere — cool crowd / evening energy */
const HERO_SRC =
  "https://images.unsplash.com/photo-1516450360452-9312f5e86fc7?auto=format&fit=crop&w=1600&q=75";
  ...
  return (
    <DiscoverMotionRoot>
      <main className="relative isolate min-h-[140dvh] overflow-x-hidden bg-[color:var(--background)] text-[color:var(--foreground)]">
        {!reduce ? (
          <WelcomeScrollMount enabled heroRef={heroRef} scrimRef={scrimRef} />
        ) : null}

        <div className="sticky top-0 isolate min-h-dvh overflow-hidden">
          <div className="absolute inset-0 -z-10">
            <div ref={heroRef} className="absolute inset-0 will-change-transform">
              <div className={`absolute inset-0 ${reduce ? "" : "welcome-hero-kenburns"}`}>
                <Image
                  src={HERO_SRC}
                  alt=""
                  fill
          ...
          >
            ...
            <m.div className="mt-8 w-full max-w-md" variants={reduce ? undefined : fadeUp}>
              <Link
                href="/discover"
            ...
            <m.p className="mt-5 text-xs text-white/55" variants={reduce ? undefined : fade}>
